...
class HanoiGame(TextGuessGame):
    ...
    def __init__(self, user_id: Snowflake, multiguesser: bool):
        ...
        self.grid = [[0 for _ in range(disk_count)] for _ in range(stack_count)]
        self.grid[0] = [i + 1 for i in range(disk_count)]
        logger.info(self.grid)

    def find_top(self, stack: int) -> int:
    ...
    def add_guess(self, user_id: Snowflake, guess: str) -> GuessOutcome:
        ...
        source_disk: int = self.find_top(source_stack)
        dest_disk: int = self.find_top(dest_stack)
        if dest_disk != disk_count and self.grid[dest_stack][dest_disk] < self.grid[source_stack][source_disk]:
        ...
        self.grid[source_stack][source_disk], self.grid[dest_stack][dest_disk] = self.grid[dest_stack][dest_disk], self.grid[source_stack][source_disk]
        logger.info(digits)
    ...
    def __str__(self) -> str:
      ...
      if self.message is not None and self.check_win():
        status += '\nYou win!'
